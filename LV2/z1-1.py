...
real_solutions = [sol for sol in solutions if all(sol[var].is_real for var in sol)]

# Filtriranje rješenja s nenegativnim slack varijablama
valid_solutions = [sol for sol in real_solutions if all(sol[var] >= 0 for var in sol if var != l1 and var != l2 and var != l3)]
...
valid_region = np.logical_and.reduce([constraint1_func(X, Y) <= 0, constraint2_func(X, Y) <= 0, constraint3_func(X, Y) <= 0])
plt.imshow(valid_region.astype(int), extent=(x_vals.min(), x_vals.max(), y_vals.min(), y_vals.max()), origin="lower", cmap="Greys", alpha=0.7)
